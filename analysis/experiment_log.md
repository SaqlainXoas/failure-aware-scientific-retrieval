...
## 2026-08-11 — Opened the held-out test split (one-time final evaluation)

- Decision: opened `beir/scifact/test` and evaluated all three pipelines exactly once. Preconditions from the pre-registration were all met beforehand: retrieval implementations passing tests, configs locked, confidence features locked, and the primary-pipeline rule (highest calibration-dev Recall@50 → `hybrid_rrf`) fixed and recorded.
- Protocol: nothing was refitted. The scaler, LogisticRegression, and Platt baseline are the calibration-train models; display thresholds were carried over from calibration-dev rather than re-chosen on test. Reaching the split requires calling `load_final_test_split`, a separate named function — `resolve_split` still refuses `test`, so no calibration code path can reach held-out data by passing a different string. 300 test queries, all with ≥1 positive qrel.
- Result (replicated): the primary-pipeline rule holds out of sample — hybrid has the highest Recall@50 on test as well (`0.9437` vs dense `0.9250`, BM25 `0.8686`). Hybrid's final top-10 success exceeds BM25's by `+0.027` with the interval excluding zero (`[+0.003, +0.050]`), a *stronger* result than on calibration-dev where the interval touched zero.
- Result (replicated): the failure-composition shift holds. Share of failures coming from the candidate set falls from `67.3%` (BM25) to `42.9%` (dense) to `34.0%` (hybrid), the same ordering and direction as calibration-dev (`76.0%` → `38.1%` → `25.0%`), with a smaller spread.
- Result (replicated): the calibrator is significantly worse calibrated than the train-fitted Platt baseline on all three pipelines — Brier `+0.048`, `+0.054`, `+0.075`, every interval entirely above zero. This is now confirmed on held-out data.
- **Result (did not replicate)**: the calibration-dev finding that reranking *significantly degrades* dense nDCG@10 (`-0.044`, `[-0.081, -0.009]`) does not hold on test, where the same comparison is `-0.017` with an interval crossing zero (`[-0.043, +0.012]`). On test no before/after reranking comparison is significant for any pipeline, and BM25 nDCG@10 actually rises `+0.022` (`[-0.008, +0.052]`). The defensible claim is therefore "reranking does not reliably improve final ranking quality here", not "it degrades it".
- **Result (underpowered, not refuted)**: the calibrator's AUROC advantage is directionally consistent on test (`+0.044`, `+0.049`, `+0.034`) but every interval crosses zero at 300 queries / ~50 failures. The cross-validated estimate over 809 calibration queries (~120 failures) had all intervals excluding zero. Held-out data agrees in direction and lacks the power to confirm it alone; both are reported.
- Consequence: the README and report were revised to weaken the reranking claim to what held-out data supports. No model, feature, threshold, or configuration was changed in response to any test result, and the split will not be evaluated again.

## 2026-08-17 — Class-weight ablation (post-hoc, calibration data only)

- Problem: the manuscript attributed the calibrator's worse-than-baseline Brier score to the predeclared `class_weight="balanced"`, but the 2026-08-11 entry recorded that no unweighted ablation had been run. The attribution was therefore an interpretation, not a measurement, and a reviewer can reasonably ask why a one-parameter experiment was left undone.
- Decision: refit the same eight common features on the same fitting split with `class_weight=None`, cross-validated on the *same* stratified folds and seed as the primary model, and bootstrap the comparison with the identical paired procedure (1,000 resamples, 95%, seed 42).
- Decision: report it in its own artifact (`results/tables/class_weight_ablation.{json,md}`) and exclude it from `confidence_comparison.md` / `selective_coverage.md`.
- Reason: the ablation was specified *after* seeing the Brier result. Keeping it in a separate table makes the predeclared/post-hoc boundary structural rather than a matter of prose, and leaves every predeclared table with exactly the rows the protocol fixed. The primary model is unchanged and no predeclared number was recomputed — verified by diffing every regenerated table: the only changes are run-directory and commit-hash provenance lines.
- Result (attribution confirmed): removing the class weighting improves Brier on every pipeline, with every interval excluding zero — BM25 `-0.060` (`[-0.074, -0.046]`), dense `-0.066` (`[-0.081, -0.050]`), hybrid `-0.070` (`[-0.084, -0.054]`). The manuscript's causal claim is now measured rather than interpreted.
- Result (stronger than expected): the unweighted model is also better calibrated than the train-fitted Platt baseline it previously lost to — BM25 `-0.012` (`[-0.019, -0.005]`), dense `-0.015` (`[-0.023, -0.006]`), hybrid `-0.012` (`[-0.019, -0.004]`), all excluding zero. The calibration deficit was not a property of the feature set.
- Result (no cost): discrimination is unchanged. AUROC differences are `+0.001`, `+0.002`, `-0.001`, every interval crossing zero. Balanced weighting mostly shifts the logistic intercept, so the induced ordering barely moves — consistent with the near-identical AUROC.
- Consequence for the paper: the predeclared result stands exactly as reported, but the *framing* of the discrimination-versus-calibration finding must weaken. The trade-off is not intrinsic to the feature set; it was an artifact of one pre-specified hyperparameter that cost probability quality and bought no discrimination. The claim that the two properties should be measured separately survives and is better supported, since a single model now attains both.
- Constraint: the held-out test split was not reopened. This ablation is calibration-data only, so the unweighted model has no held-out evaluation and must not be presented as a validated replacement for the primary model.
